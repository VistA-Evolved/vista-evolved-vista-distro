...
DINIT(SITENUMBER,SITENAME) ; Silent Dinit Replacement
 ...
 D
 . I $P($SY,",")=47 S ^DD("OS")=19 Q
 . I $L($SY,":")=2 S ^DD("OS")=18 Q
...
KSP ; Kernel System Parameters cleanup. Fall through.
 N KBANI S KBANI=0
 N GREFC S GREFC="^XTV(8989.3,1,4," ; Closed File Root for Volume Multiple
 N GREFO S GREFO="^XTV(8989.3,1,4," ; Open File Root for Volume Multiple
 F  S KBANI=$O(^XTV(8989.3,1,4,KBANI)) Q:'KBANI  D
 . N DA,DIK S DA(1)=1,DA=KBANI,DIK=GREFO D ^DIK ; Kill each entry in Vol subfile
 ;
 ; DEBUG.ASSERT - Make sure that the file is empty (non-fatal for full-data loads)
 I $O(^XTV(8989.3,1,4,0)) W !,"KBANTCLN WARNING: 8989.304 not empty after KF, continuing...",!
 ;
 N KBANFDA
 ...
 I $D(KBANERR) W !,"KBANTCLN WARNING: 8989.304 filing error, continuing...",!
 ;
 ...
 I $D(KBANERR) W !,"KBANTCLN WARNING: 8989.3 filing error, continuing...",!
 ;
F14P5 ; 14.5 clean-up. Fall through.
 ...
 N KBANERR ; For errors
 D UPDATE^DIE("E",$NA(KBANFDA),"",$NA(KBANERR)) ; File data
 I $D(KBANERR) W !,"KBANTCLN WARNING: File 14.5 filing error, continuing...",!
 ;
...
F14P7 ; 14.7 clean-up. Fall through
 ...
 I $D(KBANERR) W !,"KBANTCLN WARNING: File 14.7 filing error, continuing...",!
 ;
...
F19P2 ; 19.2 clean-up; Fall through.
 N GREFC S GREFC="^DIC(19.2," ; Closed File Root for Option Scheduling
 N GREFO S GREFO="^DIC(19.2," ; Open File Root for Option Scheduling
 ;
 ; Walk through entries
 N KBANI S KBANI=0
 F  S KBANI=$O(^DIC(19.2,KBANI)) Q:'KBANI  D
 ...
 I $D(KBANERR) W !,"KBANTCLN WARNING: File 19.2 filing error, continuing...",!
 ;
...
MSP ; Mailman Site Parameters Clean-up
 ...
 N KBANERR
 D FILE^DIE("",$NA(KBANFDA),$NA(KBANERR))
 I $D(KBANERR) W !,"KBANTCLN WARNING: MSP filing error, continuing...",!
 QUIT
 ;
...
DEVVOL ; Delete Volume field for each device.
 ...
 N KBANERR
 D FILE^DIE("",$NA(KBANFDA),$NA(KBANERR))
 I $D(KBANERR) W !,"KBANTCLN WARNING: DEVVOL filing error, continuing...",!
 QUIT
 ;
DEVNULL ; Fix up null devices
 ...
 N FDA
 N KBANI S KBANI=0
 F  S KBANI=$O(^%ZIS(1,"B","NULL",KBANI)) Q:'KBANI  S FDA(3.5,KBANI_",",.01)="ZZNULL"
 I $D(FDA) D FILE^DIE(,$NA(FDA))
 ;
 ...
 K FDA
 N KEEPNULL S KEEPNULL=+$O(^%ZIS(1,"C",NULL,0))
 I KEEPNULL D
 . S KBANI=KEEPNULL
 . F  S KBANI=$O(^%ZIS(1,"C",NULL,KBANI)) Q:'KBANI  S FDA(3.5,KBANI_",",.01)="ZZNULL"
 . I $D(FDA) D FILE^DIE(,$NA(FDA))
 ;
...
DEVHFS ; Fix up HFS device
 N OS S OS=$$VERSION^%ZOSV(1)
 N HFSSUBTYPE S HFSSUBTYPE=$$SUBIEN("P-HFS/80/99999")
 I 'HFSSUBTYPE S HFSSUBTYPE=$$SUBIEN("P-OTHER")
...
DEVTTY ; Fix TTY
 ...
 N VTIEN S VTIEN=$$SUBIEN("C-VT220")
 I 'VTIEN S VTIEN=$$SUBIEN("C-VT100")
...
DEVZERO ; Fix ZERO Device
 ...
 . S FDA(3.5,IENS,4)=1                      ; ASK DEVICE
 . S FDA(3.5,IENS,3)="`"_$$SUBIEN("P-OTHER") ; TERMINAL TYPE
 . N ERR,IEN
...
XU522 ; Disable Old CAPRI Log-in
 ...
 QUIT
 ;
DEVCIEN(VALUE) ; Find device IEN by $I value from ^%ZIS(1,"C")
 Q +$O(^%ZIS(1,"C",VALUE,0))
 ;
SUBIEN(NAME) ; Find terminal subtype IEN by exact name from ^%ZIS(2,"B")
 Q +$O(^%ZIS(2,"B",NAME,0))
 ;
 ;
...
KF(FN,IENS) ; Kill File; Private Procedure
 ...
 ; Wrap indirection in error handler -- some file roots have trailing chars
 ; that cause %YDB-E-INDEXTRACHARS on full VistA global loads.
 N %,$ETRAP S $ETRAP="S $EC="""" Q"
 S %=$G(@(GREF_"0)")) ; Save off zero node
 S $P(%,U,3,4)="" ; remove last touched and newest record markers
 K @(GREF_")") ; bye
 S @(GREF_"0)")=% ; restore zero node
 QUIT
