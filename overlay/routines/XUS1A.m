XUS1A ;SF-ISC/STAFF - SIGNON overflow from XUS1 ;12/02/14  13:26
 ...
 ; Local overlay: tolerate sparse local demo data during XU USER SIGN-ON protocol.
 Q
USER() ;
 N %B,%E,%T,I1,X1,X2
...
USX S $P(^VA(200,DUZ,1.1),U,3)=1
 ;Call XQOR to handle SIGN-ON protocol.
 ;SKIP: EN^XQOR hangs in local distro (protocol item 8944 blocks).
 ;This is safe for demo/dev — the protocol handles non-critical
 ;sign-on events (mail check, security counters, etc.).
 N XUSER,XUSQUIT,%ZTERR ;Protect ourself.
 S XUSQUIT=0
 K X,DIC
...
DUZ ;setup duz, also see XUS5
 ...
 S X=$P($G(^DIC(4,DUZ(2),99)),U,5),DUZ("AG")=$S(X]"":X,1:$P($G(^XTV(8989.3,1,0)),U,8))
