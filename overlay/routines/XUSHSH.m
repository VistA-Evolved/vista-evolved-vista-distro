XUSHSH ;ISF/STAFF - ENCRYPTION/DECRYPTION UTILITIES ; 10/30/17 5:28pm
 ...
 ;
 ; Local overlay: wrap InterSystems-specific syntax in XECUTE so YottaDB can load
 ; the routine without parser errors while preserving the OpenM path semantics.
 ;
...
SHAHASH(N,X,FLAG) ;One-Way Hash Utility, IA #6189
 ...
 I $G(^%ZOSF("OS"))["OpenM" D  Q HASH
 . X "n hash s hash=$system.Encryption.SHAHash("_N_",X)"
 . I FLAG="H" X "S HASH=##class(%xsd.hexBinary).LogicalToXSD(hash)"
...
B64ENCD(X) ;Base 64 Encode, IA #6189
 N Y
 I $G(^%ZOSF("OS"))["OpenM" D  Q Y
 . X "S Y=$System.Encryption.Base64Encode(X)"
 N %COMMAND
 S %COMMAND="base64"
 O "COMM":(SHELL="/bin/sh":COMM=%COMMAND)::"pipe"
 U "COMM" W X S $X=0 W /EOF ; $X of 0 prevents the code from writing LF to the stream
...
B64DECD(X) ;Base 64 Decode, IA #6189
 N %COMMAND,Y
 I $G(^%ZOSF("OS"))["OpenM" D  Q Y
 . X "S Y=$System.Encryption.Base64Decode(X)"
 S %COMMAND="base64 -d"
...
RSAENCR(TEXT,CERT,CAFILE,CRLFILE,ENC) ;RSA Encrypt, IA #6189
 ...
 . X "S RESULT=$system.Encryption.RSAEncrypt(TEXT,CERT,$G(CAFILE),$G(CRLFILE),ENC)"
 ;
 ; VEN/SMH:
...
RSADECR(TEXT,KEY,PWD,ENC) ;RSA Decrypt, IA #6189
 ...
 . D CLOSE^%ZISUTL("XUSHSH")
 . X "S RESULT=$system.Encryption.RSADecrypt(TEXT,KEY,$G(PWD),ENC)"
 ;
 ; VEN/SMH:
...
AESENCR(TEXT,KEY,IV) ;AES Encrypt, IA #6189
 ...
 N Y
 I $G(^%ZOSF("OS"))["OpenM" D  Q Y
 . X "S Y=$system.Encryption.AESCBCEncrypt(TEXT,KEY,IV)"
 ;
...
AESDECR(TEXT,KEY,IV) ;AES Decrypt, IA #6189
 N Y
 I $G(^%ZOSF("OS"))["OpenM" D  Q Y
 . X "S Y=$system.Encryption.AESCBCDecrypt(TEXT,KEY,IV)"
 ;
