...
VERSION() ; EXTRINSIC. ICR #5844 (supported)
 ...
 ;
 ; LOCAL OVERLAY: Wrap $SYSTEM.Process.IPv6Format() in XECUTE for YottaDB.
 ; Bare ObjectScript syntax causes a compile error on GT.M/YottaDB even though
 ; this code path only executes on InterSystems Cache.
 N %
 S %=0
 I $$VERSION^%ZOSV(1)["Cache" I +$$VERSION^%ZOSV()>2009 X "S %=$SYSTEM.Process.IPv6Format()"
 I +$SY=47 D  ; GT.M
